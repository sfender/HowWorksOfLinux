...
static void child_fun(int id, struct timespec *buf, int nrecord, unsigned long nloop_per_resol, struct timespec start)
{
    int i;
    for ( i = 0; i < nrecord; i++)
    {
...
int main(int argc, char *argv[])
{
    ...
    if (total % resol)
    {
        fprintf(stderr, "<total>(%d) should be multipul of <resolution>(%d)\n", total, resol);
        exit(EXIT_FAILURE);
    }
    ...
    puts("estimating workload which taked just one millsecond ");
    ...
    for (i = 0, ncreated = 0; i < nproc; i++, ncreated++)
    {
        ...
        {
            // children
            child_fun(i, logbuf, nrecord, nloop_per_resol, start);
